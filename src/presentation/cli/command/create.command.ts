...
import type { CreateOrUpdatePrUseCase } from "../../../application/use-case/create-or-update-pr.use-case";
...
import { CliInterfaceServiceToken, CollectContextUseCaseToken, ConfigServiceToken, ConfigureLlmUseCaseToken, CreateOrUpdatePrUseCaseToken, LintPrUseCaseToken, LlmServicesToken } from "../../../infrastructure/di/token.constant";
...
import { buildTemplateLintConfig } from "../helper/build-template-lint-config.function";
import { resolveLlmConfiguration } from "../helper/resolve-llm-configuration.function";
import { resolvePrTemplateType } from "../helper/resolve-pr-template-type.function";
...
export class CreateCommand {
	...
	async execute(options: { isJson?: boolean }): Promise<void> {
		const spinner: Ora | undefined = options.isJson ? undefined : ora("Preparing PR...").start();
		...
		try {
			...
			const createOrUpdatePr: CreateOrUpdatePrUseCase | undefined = this.CONTAINER.get<CreateOrUpdatePrUseCase>(CreateOrUpdatePrUseCaseToken);

			if (!createOrUpdatePr) {
				throw new Error("CreateOrUpdatePrUseCase not registered in container");
			}
			...
			const templateType: EPrTemplateType = await resolvePrTemplateType(context.branch, Boolean(options.isJson), cliInterface);
			const templateLintConfig: IPrLintConfig = buildTemplateLintConfig(config.lint, templateType);

			const templateContext: IPrContext = {
				...context,
				requiredSections: templateLintConfig.requiredSections,
				templateType,
			};

			if (config.github.prohibitedBranches.includes(context.branch)) {
				spinner?.fail(`Cannot create PR from prohibited branch "${context.branch}". Switch to a feature branch.`);
				process.exitCode = 1;

				return;
			}

			if (context.branch === config.github.base) {
				spinner?.fail(`Current branch "${context.branch}" is the same as target base. Switch to a feature branch.`);
				process.exitCode = 1;
			...
			const generated: { body: string; title: string } = await llmService.generate(templateContext, model);

			if (spinner) spinner.text = "Validating generated content...";

			const lintContext: IPrContext = {
				...templateContext,
				body: generated.body,
				title: generated.title,
			};

			const lintResult: IPrLintResult = lintPr.execute(lintContext, templateLintConfig, config.ticket);

			if (!lintResult.isPassed) {
				spinner?.fail("Generated PR content does not pass lint checks.");
				...
				if (options.isJson) {
					...
					cliInterface.log(chalk.bold.cyan("Generated PR (failed lint)"));
					cliInterface.log(chalk.gray("─".repeat(SEPARATOR_WIDTH)));
					cliInterface.log(`${chalk.bold("Template:")} ${templateType}`);
					cliInterface.log(`${chalk.bold("Title:")} ${generated.title}\n`);
					cliInterface.log(generated.body);
					cliInterface.log("");
					cliInterface.log(HumanPresenter.presentLintResult(lintResult));
				}
			...
			const prNumber: number = await createOrUpdatePr.execute(generated.title, generated.body, context.branch, config.github.base, config.github.isDraft);
			...
			if (options.isJson) {
				...
				cliInterface.log(`${chalk.bold("Template:")} ${templateType}`);
				cliInterface.log(`${chalk.bold("Title:")} ${generated.title}`);
				cliInterface.log(`${chalk.bold("PR:")}    #${String(prNumber)}`);
			}
		} catch (error) {
